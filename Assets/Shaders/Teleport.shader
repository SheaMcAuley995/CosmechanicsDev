Shader "Custom/Teleport" {
	Properties{
		...
		_Metalic("Metalic", 2D) = "bump" {}
		_Normal("Normal", 2D) = "bump" {}
		_DissolveAmount("Dissolve amount", Range(-3,3)) = 0
		_Direction("Direction", vector) = (-1.25, 0, 0, 0)
		[HDR]_Emission("Emission", Color) = (0.1098, 0.3215, 0.7490, 1)
		_NoiseSize("Noise size", float) = 1000
		_Stretchiness("Stretchiness", float) = -1
	}
		SubShader{
			...
			void vert(inout appdata_full v, out Input o) {
				...
				half test = ((dot(o.worldPosAdj, float3(0, _Stretchiness, 0)) + 1) / 2) - _DissolveAmount;
